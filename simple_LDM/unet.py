'''
adapted from https://github.com/lansinuote/Diffusion_From_Scratch/blob/main/3.unet.ipynb
'''
...
class CrossAttention(torch.nn.Module):
    ...
    def forward(self, q, kv):
        # x -> [1, 4096, 320] image embedding
        # 4096 = 64 * 64 size of image feature map
        # kv -> [1, 77, 768] text embedding
...
class Transformer(torch.nn.Module):
    ...
    def forward(self, q, kv):
        ...
        # 维度不变
        # [1, 4096, 320]
        q = self.atten1(q=self.norm_atten0(
            q), kv=self.norm_atten0(q)) + q  # self attenstion
        q = self.atten2(q=self.norm_atten1(q), kv=kv) + q  # cross attention
